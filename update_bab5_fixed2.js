...
const afterBab4Phrase = `"id": "kls9-smtgenap-bab1",`;

let nextIndex = content.indexOf(afterBab4Phrase);

if (nextIndex !== -1) {
    let before = content.substring(0, nextIndex);
    let after = content.substring(nextIndex);
    
    // Inject Bab 5 right before kls9-smtgenap-bab1.
    // The structure before nextIndex is `    }, {\n        `
    // We just inject `bab5Str` and another `    }, {\n        ` before `after`
    let newContent = before + bab5Str + '\n    }, {\n        ' + after;
    
    fs.writeFileSync('js/materi.js', newContent, 'utf8');
    console.log('Success repairing and injecting bab 5');
} else {
    console.error('Failed to find Genap Bab 1');
}
