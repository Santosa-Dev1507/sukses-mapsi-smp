// ΓöÇΓöÇ Helper Otomatis Memperbesar Teks Arab (Ayat/Lafal) ΓöÇΓöÇ
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ΓöÇΓöÇ Tampilkan kartu pilih topik ΓöÇΓöÇ
        ...
        soalList.forEach((soal, idx) => {
            ...
            if (hasilEl) {
                ...
                if (correct) {
                    ...
                    let kunciTxt = '';
                    if (soal.tipe === 'menjodohkan') {
                        kunciTxt = soal.kolomKiri.map((k, i) => `<br>ΓÇó ${k} Γ₧ö <strong>${soal.kolomKanan[soal.kunci[i]]}</strong>`).join('');
                    } else if (soal.tipe === 'pgk' || soal.tipe === 'pga') {
                    ...
                    hasilEl.innerHTML = `
                        <div class="flex items-start gap-2 text-red-600 font-bold mb-3">
                            ...
                            <div>Jawaban Belum Tepat ΓÇö Kunci Benar: <strong>${kunciTxt}</strong></div>
                        </div>
                        <div class="text-sm text-on-surface-variant leading-relaxed">
                            <span class="font-semibold text-on-surface">≡ƒÆí Penjelasan: </span>${formatTeksArab(soal.penjelasan)}
                        </div>`;
    ...
    function tampilHasil(benar, total, salahNomor, durasi) {
        ...
        const grade  = persen >= 90 ? '≡ƒÅå Luar Biasa!' : persen >= 75 ? '≡ƒæì Bagus!' : persen >= 60 ? 'Γ£¿ Cukup Baik' : '≡ƒôÜ Perlu Belajar Lagi';
    ...
    function kirimLaporan(benar, total, salahNomor, durasi) {
        ...
        const topik   = info ? `${info.kelas || ''} ${info.semester || ''} ${info.bab || ''} ΓÇö ${info.judul || id}`.trim() : (id || 'Latihan Harian');
