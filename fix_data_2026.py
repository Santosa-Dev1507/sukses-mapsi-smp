...
# Clear the broken quizData2026 and replace it.
# We slice text until 'const quizData2026'
match = re.search(r'const quizData2026\s*=\s*', text)
if match:
    clean_text = text[:match.start()]
    # Just append the new array and proper closing
    final_text = clean_text + "const quizData2026 = " + new_array_json + ";\n"
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(final_text)
    print("Fixed data.js syntax and injected 10 questions successfully.")
else:
    print("Could not find quizData2026 declaration.")
